...
impl<T: 'static + AsLogicalPlan, U: 'static + AsExecutionPlan> SchedulerState<T, U> {
    ...
    pub(crate) async fn submit_job(
        ...
        let warnings = session_ctx.state().config().get_extension::<WarningCollector>().map(|w| w.warnings(job_id.to_owned())).unwrap_or_default();

        self.task_manager
            .submit_job(job_id, job_name, &session_ctx.session_id(), plan, queued_at, warnings)
            .await?;
